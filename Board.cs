...
//state machine to enamble the player to move
...
//set the x and y position and the tile kind
[System.Serializable]
...
public class Board : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach(TileType tile in boardLayout)
        {
            Debug.LogError(tile.tileKind);
        }
    }
    ...
    private void GenerateChocolateTiles()
    {
        ...
        for (int i = 0; i < boardLayout.Length; i++)
        {
            ...
            if (boardLayout[i].tileKind == TileKind.Chocolate)
            {
                //Create a "Lock" tile at that position;
                Vector2 tempPosition = new Vector2(boardLayout[i].x, boardLayout[i].y);
                GameObject tile = Instantiate(chocolatePiecePrefab, tempPosition, Quaternion.identity);
    ...
    private Vector2 CheckForAdjacent(int column, int row)
    {
        ...
        //if (column < width - 1 && allDots[column + 1, row])" instead of "if (allDots[column + 1, row] && column < width - 1)" 
    ...
    private void MakeNewChocolate()
    {
        ...
        while (!chocolate && loops < 200)
        {
            ...
            if (chocolateTiles[newX, newY] != null)
            {
                Vector2 adjacent = CheckForAdjacent(newX, newY);
                Debug.LogWarning(adjacent);
